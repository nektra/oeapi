/* $Id: main.cpp,v 1.65.2.1.4.17 2007/08/30 19:40:11 ibejarano Exp $
 *
	...
	switch(msg)
	{
		case WM_WINDOWPOSCHANGING:
		{
			...
			if(wp && wp->flags & SWP_SHOWWINDOW) {
				// Double cast to avoid warning about pointer truncation
				BOOL bSkip = (BOOL)(LONGLONG)::GetProp(hwnd, _T("NKT#SNDRCV"));
				if(bSkip) {
					::SetProp(hwnd, _T("NKT#SNDRCV"), (HANDLE)FALSE);
		...
		case SEND_MSG_CODE:
		{
			...
			//else {
			//	debug_print(DEBUG_TRACE, _T("SendDlgWndProc: pass %08x, %08x - %d.\n"), wParam, lParam, ::GetCurrentThreadId());
			//}

#ifdef ENTERPRISE_VERSION
			//DWORD maxId = -1;

			// TODO: Check if we have a new message in outbox 
			//maxId = OEAPIManager::Get()->GetOutboxMaxId();
			/* maxId = GetFolderMaxMsgID(OEAPIManager::Get()->GetOutboxFolder());

			if(OEAPIManager::Get()->GetLastOutboxMsgProcessed() == -1 && maxId != -1 ||
				maxId != -1 && maxId > OEAPIManager::Get()->GetLastOutboxMsgProcessed()) {
				return TRUE;
			} */
...
OEAPIManager::OEAPIManager()
{
	...
	//pThis = NULL;
	menusIDs_ = 0;
	lastFolderProp_.idFolderNew = lastFolderProp_.idFolderOld = -1;
	msgTable_ = NULL;
	msgList_ = NULL;
	msgListWMail_ = NULL;
	curFolderId_ = -1;
...
void OEAPIManager::InitializeMessageStore()
{
	...
	oeapiMsgList_ = new OEAPIMessageList;
	oeapiMsgList_->Init();

	if(IsWMail()) {
		msgStore_ = new OEAPIMessageStoreWM();
	}
	else {
		msgStore_ = new OEAPIMessageStoreOE();
	}
...
BOOL OEAPIManager::CreateMessageStore()
{
	...
	HRESULT hr = CoCreateInstance(CLSID_StoreNamespace, NULL,
		CLSCTX_SERVER, IID_IStoreNamespace, (LPVOID*) &pSN);
	...
	hr = pSN->OpenSpecialFolder(FOLDER_OUTBOX, 0, &outbox);
	if(FAILED(hr)) {
		pSN->Release();
		pSN = NULL;
...
void OEAPIManager::SetSelectedFolderName(HWND hTree)
{
	...
	OEAPIMessageStore::FolderType type;
	if(msgStore_ != NULL) {
...
void OEAPIManager::CreateOutboxMessageOE(int folderId, int msgId, BOOL deleteAfter)
{
	...
	if(!GetDefaultAccountSettingsOE(&info)) {
		debug_print(DEBUG_ERROR, _T("OEAPIManager::CreateOutboxMessageOE: GetDefaultAccountSettings.\n"));
		return;
	}

	hr = msgStore_->OpenMimeMessage(folderId, msgId, &pMimeMsg);
	if(FAILED(hr)) {
		debug_print(DEBUG_ERROR, _T("OEAPIManager::CreateOutboxMessageOE: OpenMessage.\n"));
		return;
	}
	...
	if(FAILED(hr)) {
		...
		if(FAILED(hr)) {
			debug_print(DEBUG_ERROR, _T("OEAPIManager::CreateOutboxMessageOE: Set account id failed %08x.\n"), hr);
			return;
		}
		...
		hr = pMimeMsg->SetBodyProp(HBODY_ROOT, _T("att:athena-account-name"), NOFLAGS, &value);
		if(FAILED(hr)) {
			debug_print(DEBUG_ERROR, _T("OEAPIManager::CreateOutboxMessageOE: Set account name failed %08x.\n"), hr);
			return;
		}
	}
	else {
		// Copy account info from message
		_tcscpy_s(info.accountId, sizeof(info.accountId), value.pszVal);
		PropVariantClear(&value);

		hr = pMimeMsg->GetBodyProp(HBODY_ROOT, _T("att:athena-account-name"), NOFLAGS, &value);
		if(FAILED(hr)) {
			debug_print(DEBUG_ERROR, _T("OEAPIManager::CreateOutboxMessageOE: GetBodyProp failed %08x.\n"), hr);
			return;
		}
		_tcscpy_s(info.accountName, sizeof(info.accountName), value.pszVal);		
		PropVariantClear(&value);
	}

	if(folderId != outboxFolderId_)
	{
		hr = msgStore_->SaveMimeMessage(outboxFolderId_, 0x00010089, pMimeMsg, &newMsgId);
		if(FAILED(hr)) {
			debug_print(DEBUG_ERROR, _T("OEAPIManager::CreateOutboxMessageOE: SaveMimeMessage failed %08x.\n"), hr);
			return;
		}

		if(deleteAfter) {
			hr = msgStore_->DeleteMessage(folderId, msgId, TRUE);
		}
		return;
	}

	hr = pMimeMsg->Commit(COMMIT_REUSESTORAGE);
	if(FAILED(hr)) {
		debug_print(DEBUG_ERROR, _T("OEAPIManager::CreateOutboxMessageOE: Commit failed %08x.\n"), hr);
		return;
	}

	IMessageFolderPtr msgFolder;
	hr = msgStore_->OpenFolder(outboxFolderId_, &msgFolder);
	if(FAILED(hr)) {
		debug_print(DEBUG_ERROR, _T("OEAPIManager::CreateOutboxMessageOE: OpenFolder failed %08x.\n"), hr);
		return;
	}

	MESSAGEINFO msgInfo = {0};
	msgInfo.dwMessageId = msgId;
	hr = msgFolder->FindRecord(0, -1, &msgInfo, NULL);
	if(FAILED(hr)) {
		debug_print(DEBUG_ERROR, _T("OEAPIManager::CreateOutboxMessageOE: FindRecord failed %08x.\n"), hr);
		return;
	}

	LPSTR accountName = msgInfo.pszAthenaAccountName;
	LPSTR accountId = msgInfo.szServerName;

	msgInfo.pszAthenaAccountName = info.accountName;
	msgInfo.szServerName = info.accountId;

	msgInfo.dwARF |= 0x000010089;

	hr = msgFolder->UpdateRecord(&msgInfo);

	msgInfo.pszAthenaAccountName = accountName;
	msgInfo.szServerName = accountId;

	msgFolder->FreeRecord(&msgInfo);
	if(FAILED(hr)) {
		debug_print(DEBUG_ERROR, _T("OEAPIManager::CreateOutboxMessageOE: UpdateRecord failed %08x.\n"), hr);
		return;
	}
}
...
void OEAPIManager::CreateOutboxMessageWM(int folderId, int msgId, BOOL deleteAfter)
{
	...
	LPSTR accountId = msgInfo.szAccountGUID;
	LPWSTR accountName = msgInfo.wzAccountName;
	LPWSTR displayFrom = msgInfo.wzDisplayFrom;
	LPSTR sender = msgInfo.szSender;
								...
								DWORD folId, DWORD msgId1, DWORD arf1, DWORD msgId2, DWORD arf2)
{
	if(ft == folder_local_mail) {
		if((txType & oeapi_tx_insert) ||
			...
			tx->msgId = msgId1;
			tx->txType = txType;
			tx->parentId = (txType == oeapi_tx_folder_move) ? msgId2 : -1;

			//::PostMessage(hWnd_, DATABASE_TRANSACTION_CODE, (WPARAM)tx, 0);
			HANDLE hEvent = ::CreateEvent(NULL, TRUE, FALSE, NULL);
			//ResetGUIFlag();
			::PostMessage(hWnd_, DATABASE_TRANSACTION_CODE, (WPARAM)tx, (LPARAM)hEvent);
			//::PostMessage(hWnd_, DATABASE_TRANSACTION_CODE, (WPARAM)tx, NULL);
			DWORD res;
			MSG msg;
			while(true) {
			...
			//debug_print(DEBUG_TRACE, _T("OEAPIManager::OnDatabaseTransaction: before %x - %d.\n"), txType, ::GetCurrentThreadId());
			//::SendMessage(hWnd_, DATABASE_TRANSACTION_CODE, (WPARAM)tx, 0);
			//debug_print(DEBUG_TRACE, _T("OEAPIManager::OnDatabaseTransaction: after %x.\n"), txType);
...
//			tx.txType = txType;
			...
			// we are in the COM server thread so we don't need to post the message
//			RaiseOnDatabaseChange((comet::OEAPI::tagOE_DATABASE_TRANSACTION) txType, folId, msgId1);
...
void OEAPIManager::RaiseOnDatabaseChange(OEStoreTransactionType dt, int folderId, int msgId, int parentId)
{
	...
	if(dt == OE_TR_INSERT_MESSAGE && folderId == outboxFolderId_) {
		if(OEAPIManager::Get()->GetSendDlg()) {
			g_sendDlgSyncblocked = TRUE;

			OEAPIManager::Get()->RaiseOnNewMessageInOutbox(msgId);

			OEAPIManager::Get()->SetLastOutboxMsgProcessed(msgId);

			g_sendDlgSyncblocked = FALSE;

			// If mail flagged as Send Later then do not issue Send and Receive
			if(OEAPIManager::Get()->GetSendLater())
			{
				// Reset flag
				OEAPIManager::Get()->SetSendLater(FALSE);
			}
			else if(OEAPIManager::Get()->GetSendMailImmediatelyKeyValue())
			{
...
BOOL OEAPIManager::RemoveMessageWMail(int msgId) 
{
	...
	hr = deletedItems->OpenMessage(msgId, 0, 0, &pMimeMsg, NULL);
	if(FAILED(hr)) {
...
void LoadPluginFromString(LPCTSTR valueData) 
{
	...
	if(mod) {
		...
		LoadedPluginsCount++;
		FARPROC pInit = GetProcAddress(mod, _T("InitializePlugin"));
		if(pInit) {
			pInit();
		}
		return;
	...
	if(FAILED(hr)) {
		...
		LoadedPlugins[LoadedPluginsCount].modHandle = pUnknown;
...
void UnloadRegisteredPlugins()
{
	for(DWORD i=0; i<LoadedPluginsCount; i++) {
		if(LoadedPlugins[i].type == oeapi_dll_plugin) {
			FreeLibrary((HMODULE) LoadedPlugins[i].modHandle);
		}
		else if(LoadedPlugins[i].type == oeapi_iunknown_plugin) {
			IUnknown *pPlugin = (IUnknown *) LoadedPlugins[i].modHandle;
			pPlugin->Release();
		}
...
//---------------------------------------------------------------------------//
DWORD _stdcall ServerProc(void *)
{
	HRESULT hr;
...
void _stdcall StartServer(HWND hwnd)
{
	if(hServer == NULL) {
		...
		if(hwnd) {
			...
			if((hServer = CreateThread(0, 0, ServerProc, 0, 0, &serverID)) == NULL) {
...
STDAPI DllRegisterServer()
{
	...
	if(RegOpenKeyEx(HKEY_CLASSES_ROOT, keyName.s_str().c_str(),
		0, KEY_ALL_ACCESS, &hKey) == NO_ERROR) {
		HMODULE hMod = LoadLibrary("Shlwapi");
		if(hMod) {
